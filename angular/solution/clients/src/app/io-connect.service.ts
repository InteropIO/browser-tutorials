...
export class IOConnectService {
    ...
    public async openStocksWindow(): Promise<void> {
        ...
        await this.ioConnectStore
            .getIOConnect()
            .windows.open(name, "http://localhost:4100");
    }
    ...
    public async sendSelectedClientByInterop(client: Client): Promise<void> {
        ...
        const interopMethod = this.ioConnectStore
            .getIOConnect()
            .interop.methods()
        ...
        await this.ioConnectStore
            .getIOConnect()
            .interop.invoke(methodName, methodArgs);
    }
    ...
    public async sendSelectedClient(client: Client): Promise<void> {
        ...
        await this.ioConnectStore
            .getIOConnect()
            .contexts.update("SelectedClient", client);
    ...
    public async startStocksApp(): Promise<void> {
        const stocksApp = this.ioConnectStore
            .getIOConnect()
            .appManager.application("Stocks");
        ...
        await stocksApp
            .start(undefined, { channelId: currentChannel?.name })
            .catch(console.error);
    }

    public async restoreWorkspace(client: Client): Promise<void> {
        try {
            const workspace = await this.ioConnectStore
                .getIOConnect()
                .workspaces?.restoreWorkspace("Client Space", {
                    context: client
                });

            await this.raiseNotificationOnWorkspaceOpen(
                client.name,
                workspace as IOConnectWorkspaces.Workspace
            );
        } catch (error: any) {
    ...
    private async raiseNotificationOnWorkspaceOpen(
        clientName: string,
        ...
        const notification = await this.ioConnectStore
            .getIOConnect()
            .notifications.raise(options);
